...
# Caddy Service Management Script
# Usage: ./manage-caddy.sh [deploy|update|restart|status|logs|stop|start]
...
deploy() {
    ...
    ssh ${SERVER_USER}@${SERVER_HOST} "sudo mkdir -p ${REMOTE_PATH}/data ${REMOTE_PATH}/config"
    ...
    scp ${SERVICE_PATH}/Dockerfile ${SERVER_USER}@${SERVER_HOST}:${REMOTE_PATH}/
    scp ${SERVICE_PATH}/Caddyfile ${SERVER_USER}@${SERVER_HOST}:${REMOTE_PATH}/
    ...
    # Deploy the service
    ssh ${SERVER_USER}@${SERVER_HOST} "cd ${REMOTE_PATH} && sudo -E docker compose --env-file .env up -d --build"
...
update() {
    ...
    # Restart the service
    ssh ${SERVER_USER}@${SERVER_HOST} "cd ${REMOTE_PATH} && sudo docker compose down && sudo -E docker compose --env-file .env up -d --build"
