...
ssh gooral@192.168.1.252 "sudo mkdir -p /opt/compose/caddy/conf /opt/compose/pihole/etc-pihole /opt/compose/pihole/etc-dnsmasq.d"
...
scp -r caddy/conf/* gooral@192.168.1.252:/opt/compose/caddy/conf/

# Copy pihole directory content (even if empty)
...
ssh gooral@192.168.1.252 "ls -la /opt/compose/ && sudo chmod -R 644 /opt/compose/.env* && sudo chmod 755 /opt/compose/caddy/conf/"
...
ssh gooral@192.168.1.252 "echo 'ENV contents:' && cat /opt/compose/.env"
