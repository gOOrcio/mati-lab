...
| **Virtual key — `openclaw`** | OpenClaw config (in-app via `openclaw config`) | `homelab/litellm/openclaw` | OpenClaw LLM provider config. | (1) LiteLLM `/key/regenerate/key_alias/openclaw`. (2) `openclaw config set ...` from container shell (Apps → openclaw → Shell). (3) PM. | 2026-04-30 (issued, awaiting in-app cutover) |
