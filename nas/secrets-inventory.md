...
| **Bearer — `vault-rag-mcp`** | `/mnt/fast/databases/vault-rag-mcp/.env` (`MCP_BEARER_TOKEN=`) on NAS. Hermes-side copy: `/mnt/fast/databases/hermes/data/config.yaml` literal value in `mcp_servers.vault-rag.headers.Authorization` (NOT env-interpolated; `${VAR}` doesn't fire inside `headers`). | `homelab/vault-rag-mcp/bearer-token` | vault-rag-mcp ASGI bearer middleware. Without matching token: every Hermes request gets 401. | (1) `openssl rand -hex 24` — prefix `vrm-`. (2) Edit `/mnt/fast/databases/vault-rag-mcp/.env`. (3) Edit `/mnt/fast/databases/hermes/data/config.yaml`. (4) `midclt app.update vault-rag-mcp && app.redeploy hermes`. (5) PM. | 2026-05-01 (issued at install) |
| **Bearer — `qbittorrent-mcp`** | `/mnt/fast/databases/qbittorrent-mcp/.env` (`MCP_BEARER_TOKEN=`) on NAS. Hermes-side copy: `/mnt/fast/databases/hermes/data/config.yaml` literal value in `mcp_servers.qbittorrent.headers.Authorization`. | `homelab/qbittorrent-mcp/bearer-token` | qbittorrent-mcp ASGI bearer middleware. Same shape as vault-rag-mcp bearer. | Same procedure as vault-rag-mcp bearer above (substitute `qbm-` prefix and `qbittorrent-mcp` paths). | 2026-05-01 (issued at install) |
| **Virtual key — `dev-pc-tools-v2`** | Dev PC `~/.claude.json` (inline in the `vault-rag` MCP server `env`) | `homelab/litellm/dev-pc-tools` | Claude Code MCP `vault-rag` tool, OpenCode, ad-hoc curl. Scope: models `agent-default, agent-smart, coding, embeddings`; budget $30/30d. | **Note: the alias route `/key/regenerate/key_alias/<alias>` 404s on this LiteLLM build.** Working procedure: (1) `GET /key/info?key=<old>` with the master key to capture alias/models/budget. (2) `POST /key/generate` with those params. (3) `POST /key/delete {"keys":["<old>"]}`. (4) `POST /key/update` if the new key needs the scope re-applied (generate does **not** inherit it). (5) Swap the value in `~/.claude.json`. (6) **Restart Claude Code** — the running MCP server holds the old key in its env. (7) PM. | 2026-09-06 (**emergency rotation**: the previous key was printed by a process listing into a session transcript; old key deleted and verified 401) |
...
| **SMTP API key (Resend)** | Pi `/opt/mati-lab/network/authelia/data/smtp_password.txt` (0600 root; templated into `notifier.smtp.password`). **DR gap:** unlike the OIDC secrets it has no copy in the local repo `network/authelia/data/`, so a rebuilt Pi will not get it back automatically — drop the key there (gitignored) to close this; `manage-authelia.sh` already copies it when present. | `homelab/authelia/smtp-resend` | Authelia outbound mail: password-reset + 2FA re-enrollment verification links. Without it Authelia **fails its startup check and refuses to boot**, which 502s every forward_auth vhost — pre-flight the credential before deploying. Sender `auth@mati-lab.online`; the Resend domain must stay verified (SPF/DKIM in Cloudflare). | (1) resend.com → API Keys → roll (sending scope). (2) Stage without exposing it in argv: `read -rs K && printf '%s' "$K" \| ssh gooral@192.168.1.252 'sudo install -m 600 -o root -g root /dev/stdin /opt/mati-lab/network/authelia/data/smtp_password.txt' && unset K`. (3) Pre-flight with `python3 smtplib` from the Pi against `delivered@resend.dev` **before** redeploying. (4) `cd network && make deploy-authelia`. (5) Confirm `Startup complete` and no errors in `docker logs authelia`. (6) PM. | 2026-09-06 (minted + staged) |
...
| **nas-crons token** | NAS `/root/.backup-env` (`NTFY_CRON_TOKEN=`) | `homelab/ntfy/nas-crons` | NAS cron scripts publishing alerts directly to ntfy (`vault-staleness-check.sh`; future drift-check). ntfy user `nas-crons`, write-only on `homelab-alerts`; login password random-discarded at mint. | (1) On Pi: `docker exec ntfy ntfy token remove nas-crons <old>` then `token add nas-crons`. (2) Update `NTFY_CRON_TOKEN=` line in NAS `/root/.backup-env` (stage via `read -rs` + `sudo tee`, or the midclt cron channel). (3) Verify: run `vault-staleness-check.sh`. (4) PM. | 2026-09-06 (minted; **re-rotate: value transited a Claude session**) |
| **Alertmanager token** | Pi `network/alertmanager/ntfy-token` (0600, gitignored; bind-mounted to `/etc/alertmanager/ntfy-token`) | `homelab/ntfy/alertmanager` | Alertmanager → ntfy `homelab-alerts` webhook (`http_config.authorization.credentials_file`). Without it: every notify 403s silently (bit us 2026-06 → 2026-09). ntfy user `alertmanager`, write-only ACL on `homelab-alerts`; user's login password was random-discarded at mint (token is the only credential). | (1) `ssh gooral@192.168.1.252`. (2) `docker exec ntfy ntfy token list alertmanager` → `docker exec ntfy ntfy token remove alertmanager <old-tk>`. (3) `TOKEN=$(docker exec ntfy ntfy token add alertmanager \| grep -oE 'tk_[A-Za-z0-9]+')`. (4) `umask 077; printf '%s' "$TOKEN" > /opt/mati-lab/network/alertmanager/ntfy-token`. (5) `cd /opt/mati-lab/network && docker compose -f alertmanager/docker-compose.yml up -d --force-recreate alertmanager`. (6) Verify: `curl -s -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $TOKEN" -d test http://localhost:8093/homelab-alerts` → 200, and Alertmanager logs show no 403. (7) PM. | 2026-09-06 (minted — fixes 3-month silent 403 outage) |
...
| Role | File on disk | PM label | Dependents | Procedure | Last rotated |
|---|---|---|---|---|---|
| **CouchDB admin password** | App compose env `COUCHDB_PASSWORD` (via `app.update obsidian-couchdb`, full-blob replace) + NAS `/root/.backup-env` (`COUCHDB_ADMIN_USER`/`COUCHDB_ADMIN_PASSWORD`, consumed by the nightly `obsidian-couchdb-dump.sh` cron). **No longer embedded in the cron command** (was visible in `cronjob.query`/ps until 2026-09-06). | `homelab/obsidian/couchdb-admin` | Nightly `_all_docs` dump cron + CouchDB admin operations; **NOT** the LiveSync plugin (separate `livesync` user). | (1) New value. (2) `curl -u admin:<old> -X PUT http://127.0.0.1:30015/_node/_local/_config/admins/admin -d '"<new>"'` on NAS. (3) `app.update obsidian-couchdb` with the full compose blob, `COUCHDB_PASSWORD` swapped (fetch via `app.config`, edit, resubmit — never partial). (4) Update both lines in `/root/.backup-env`. (5) Verify: `curl -u admin:<new> http://127.0.0.1:30015/_up` + run the dump script. (6) PM. | 2026-09-06 (rotated during sweep; **re-rotate: value transited a Claude session**) |
