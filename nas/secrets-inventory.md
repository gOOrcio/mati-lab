...
| **SMTP API key (Resend)** | Pi `network/authelia/data/smtp_password.txt` (0600, gitignored; `{{ secret }}`-templated into `notifier.smtp.password`) | `homelab/authelia/smtp-resend` | Authelia outbound mail: password-reset + 2FA re-enrollment identity-verification emails. Without it: those flows fail (or fall back to nothing). Sender `auth@mati-lab.online`; Resend domain must stay verified (SPF/DKIM DNS in Cloudflare). | (1) resend.com → API Keys → roll (sending scope). (2) `umask 077; printf '%s' '<key>' > /opt/mati-lab/network/authelia/data/smtp_password.txt` on Pi. (3) `cd network && make deploy-authelia`. (4) Verify: trigger a password reset at the portal, confirm mail arrives. (5) PM. | (pending first mint) |
