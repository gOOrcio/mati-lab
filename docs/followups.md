...
| # | Item | Origin | Source |
|---|---|---|---|
| net.1 | **VLAN segmentation of the flat LAN.** Today one corporate network (`Default`, 192.168.1.0/24) holds the G5 Flex camera, all IoT (Govee, Daikin ×3, Aqara, fridge, LG TV, JBL, Switch) *and* Proxmox / TrueNAS / the Pis. Full staged design — target VLANs, zone policy matrix, per-stage rollback, and the homelab-specific gotchas (mDNS reflection, Homebridge→IoT reachability, ambiguous DHCP ownership between UDR and Pi-hole) — is written up and ready to execute. **Key decision already made:** infrastructure stays on VLAN 1; the *clients* move, because 192.168.1.x is hardcoded across Caddy, Pi-hole, Ansible, Kuma, Prometheus, NFS and restic. **Trigger:** appetite for a multi-evening change with IoT re-onboarding. | 2026-08-13 network goal | `docs/superpowers/specs/2026-08-13-network-segmentation-design.md` (local), `network/unifi/notes.md` |
| net.2 | **Zone-Based Firewall not enabled** on the UDR — still on legacy rules (two, both `accept`, for Pi-hole DNS/DHCP). Prerequisite for net.1; also worth doing standalone since it's the modern policy model. Stage 0 of the segmentation plan. | 2026-08-13 network goal | `network/unifi/notes.md` |
